describe EthereumTx::Key, type: :model do
  let(:priv) { nil }
  subject(:key) { EthereumTx::Key.new priv: priv }

  describe "#initialize" do
    it "returns a key with a new private key" do
      key1 = EthereumTx::Key.new
      key2 = EthereumTx::Key.new
    ...
    it "regenerates an old private key" do
      key1 = EthereumTx::Key.new
      key2 = EthereumTx::Key.new priv: key1.private_hex
  ...
  describe "#sign" do
    ...
    it "signs a message so that the public key is recoverable" do
      10.times do
        ...
        s_value = EthereumTx::Utils.v_r_s_for(signature).last
        expect(s_value).to be < (EthereumTx::SECP256K1_N/2)
      end
    end
  end
